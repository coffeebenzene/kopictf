...
def validate_signed_dict(signed_dict):
    """ signed_dict : signed_dict generated by create_signed_dict()
        return : bool (True if signature is valid.) 
        
        DOES NOT CHECK IF certificate is valid. 
        Assumes the the certificate in the dict is valid.
    """
    data_dict = signed_dict.copy()
    signature = data_dict.pop("signature")
    cert = data_dict.pop("cert")
    cert = Certificate(**cert)
    
    msg = json.dumps(data_dict, sort_keys=True)
    return  validate_rsa(msg, signature, cert.rsa_pub)
